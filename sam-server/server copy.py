...
from robust_segment_anything import sam_model_registry, SamAutomaticMaskGenerator, SamPredictor
import numpy as np
import cv2
import base64
...
CHECKPOINT_PATH = "./robustsam_checkpoint.pth"

DEVICE = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
print(f"Device: {DEVICE}")
MODEL_TYPE = "vit_l"

model = sam_model_registry[MODEL_TYPE](checkpoint=CHECKPOINT_PATH, opt=None)
...
def process_folder():
    ...
    for file in list_of_files:
        ...
        embedd = predictor.get_embedding(image_cv2)
        # Serialize the embeddings
        with open(f'{folder_path}/{file}.pkl', 'wb') as f:
            pkl.dump(embedd, f)

    return jsonify({'message': 'Embeddings generated and serialized successfully'})
